...
function normalizeAppUrl(raw: string | null | undefined) {
  if (!raw) return null;
  try {
...
function createCronSecret(length = 32) {
...
export async function POST(req: Request) {
  ...
  const currentOrigin = normalizeAppUrl(new URL(req.url).origin);
  const appUrl =
    normalizeAppUrl(vercel.appUrl) ||
    currentOrigin ||
    normalizeAppUrl(process.env.NEXT_PUBLIC_APP_URL) ||
    null;
  const cronSecret = createCronSecret();
  ...
  const firstName = (admin.adminName || admin.companyName).split(' ')[0] || 'você';
  ...
  const sendLog = async (
    ...
    await sendEvent({ type: 'log', message, level, stepId });
  };

  const sendStepComplete = async (stepId: string, message?: string) => {
    await sendEvent({ type: 'step_complete', stepId, message });
  };
  ...
  (async () => {
    ...
    try {
      ...
      if (skippedSteps.length > 0) {
        ...
        await sendLog(`Etapas reaproveitadas: ${skippedSteps.join(', ')}`, 'info');
      }

      await sendLog('Sessao de instalacao iniciada.', 'info');
      ...
      if (needsKeys) {
        ...
        await sendLog('Chaves do Supabase resolvidas automaticamente.', 'info', 'resolve_keys');
      } else {
        await sendLog('Usando chaves do Supabase fornecidas pelo instalador.', 'info', 'resolve_keys');
      }
      ...
      if (needsDb) {
        ...
        await sendLog('Conexao de banco do Supabase resolvida automaticamente.', 'info', 'resolve_keys');
      } else {
        await sendLog('Usando conexao de banco informada manualmente.', 'info', 'resolve_keys');
      }

      await sendPhase('resolve_keys'); // Complete
      await sendStepComplete('resolve_keys', 'Credenciais e acesso ao banco confirmados.');
      ...
      await upsertProjectEnvs(
        ...
        [
          ...
          ...(appUrl ? [{ key: 'NEXT_PUBLIC_APP_URL', value: appUrl, targets: envTargets }] : []),
          { key: 'CRON_SECRET', value: cronSecret, targets: envTargets },
        ],
        vercel.teamId || undefined
      );

      await sendPhase('setup_envs'); // Complete
      await sendStepComplete(
        'setup_envs',
        appUrl
          ? `Ambiente configurado. URL publica definida como ${appUrl}.`
          : 'Ambiente configurado. Defina NEXT_PUBLIC_APP_URL manualmente depois do setup.'
      );
      await sendLog('Segredo de cron configurado automaticamente para rotas agendadas.', 'success', 'setup_envs');

      // Step: wait_project (skippable)
      if (!skippedSteps.includes('wait_project')) {
        await sendPhase('wait_project', 0);
        await sendLog('Aguardando o projeto Supabase ficar ativo...', 'info', 'wait_project');
        ...
        if (resolvedAccessToken && resolvedProjectRef) {
          ...
          while (Date.now() - startTime < timeoutMs) {
            const projectStatus = await getSupabaseProject({
              accessToken: resolvedAccessToken,
              projectRef: resolvedProjectRef,
            });

            if (projectStatus.ok) {
              const normalizedStatus = String(projectStatus.project.status || 'UNKNOWN').toUpperCase();
              if (normalizedStatus !== lastStatus) {
                lastStatus = normalizedStatus;
                await sendLog(`Status do Supabase: ${normalizedStatus}`, 'info', 'wait_project');
              }

              if (normalizedStatus.startsWith('ACTIVE')) {
                ready = true;
                break;
              }
            }
          ...
          if (!ready) {
            throw new Error(
              `Projeto Supabase ainda esta subindo (${lastStatus || 'status desconhecido'}). Aguarde um pouco e tente novamente.`
            );
          }
        }
        
        await sendPhase('wait_project'); // Complete
        await sendStepComplete('wait_project', 'Projeto Supabase ativo e pronto para instalacao.');
      }
      ...
      if (!skippedSteps.includes('wait_storage') || !skippedSteps.includes('migrations')) {
        if (!skippedSteps.includes('wait_storage')) {
          ...
          await sendLog('Validando Storage do Supabase antes de aplicar o schema...', 'info', 'wait_storage');
        }
        ...
        if (!skippedSteps.includes('migrations')) {
          ...
          await withRetry(
            'migrations',
            async () => {
              const schemaStatus = await runSchemaMigration(resolvedDbUrl, {
                onProgress: async (event) => {
                  const action =
                    event.kind === 'start' ? 'Aplicando' : 'Aplicada';
                  await sendLog(
                    `${action} migration ${event.index}/${event.total}: ${event.description}`,
                    event.kind === 'start' ? 'info' : 'success',
                    'migrations'
                  );
                },
              });
              if (!schemaStatus.complete) {
                throw new Error(
                  `Ainda restam migrations pendentes: ${schemaStatus.pendingIds.join(', ')}`
                );
              }
            },
            sendEvent,
          ...
          await sendStepComplete('migrations', 'Estrutura completa do CRM aplicada no banco.');
        }
        
        if (!skippedSteps.includes('wait_storage')) {
          await sendPhase('wait_storage'); // Complete (done as part of migrations)
          await sendStepComplete('wait_storage', 'Storage do Supabase confirmado.');
        }
      }
      ...
      if (supabase.deployEdgeFunctions && hasLocalEdgeFunctions) {
        await sendLog('Configurando segredos das Edge Functions...', 'info', 'edge_functions');
        await withRetry(
          'edge_secrets',
          async () => {
        ...
        const failedFunctions = (functions || []).filter((item) => !item.ok).length;
        if (failedFunctions > 0) {
          await sendLog(
            `Edge Functions publicadas com ${failedFunctions} falha(s). Revise o painel do Supabase se necessario.`,
            'warning',
            'edge_functions'
          );
        } else {
          await sendLog(
            `Edge Functions publicadas com sucesso (${functions?.length || 0} no total).`,
            'success',
            'edge_functions'
          );
        }
        ...
        await sendLog('Nenhuma Edge Function local precisou ser publicada.', 'info', 'edge_functions');
      }
      await sendStepComplete('edge_functions', 'Camada de funcoes e comunicadores finalizada.');
      ...
      if (!skippedSteps.includes('bootstrap')) {
        ...
        await withRetry(
          ...
          async () => {
            ...
            if (login1.ok) {
              ...
              await sendLog(
                'Login do administrador ja estava valido. Nenhum ajuste de credenciais foi necessario.',
                'success',
                'bootstrap'
              );
              return;
            ...
            await sendLog('Criando ou ajustando a conta administradora...', 'info', 'bootstrap');
        ...
        await sendStepComplete('bootstrap', 'Conta administradora validada para o primeiro login.');
      }
      ...
      try {
        ...
        await sendLog(
          `Redeploy iniciado na Vercel (deployment ${vercelDeploymentId}).`,
          'info',
          'redeploy'
        );
      } catch (err) {
      ...
      await sendLog('Deployment da Vercel ficou READY.', 'success', 'redeploy');
      ...
      await sendLog('Instalador desabilitado automaticamente para proteger a instancia.', 'success');
      ...
      await sendStepComplete('redeploy', 'Aplicacao pronta para o primeiro acesso.');
