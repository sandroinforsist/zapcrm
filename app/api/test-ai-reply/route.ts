...
export async function GET(request: Request) {
  ...
  try {
    ...
    const memoryContext = memories.length > 0
      ...
      : '';
    const brandRuntime = await getOrganizationBrandRuntime(supabase, orgId);
    ...
    const systemPrompt = [
      ...
      '- NUNCA colete dados de reserva pelo WhatsApp.',
      bookingLink
        ? `- Para reservas, SEMPRE direcione ao link oficial: ${bookingLink}`
        : '- Para reservas, use as ferramentas e informe que o link oficial será enviado pela equipe.',
      memoryContext,
    ].filter(Boolean).join('\n');
