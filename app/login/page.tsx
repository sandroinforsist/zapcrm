...
export default function LoginPage() {
    ...
    useEffect(() => {
        const pausedError = sessionStorage.getItem('crm_login_error')
        if (pausedError) {
            setError(pausedError)
            sessionStorage.removeItem('crm_login_error')
        }
    }, [])
            ...
            <div className="max-w-md w-full relative z-10 px-4">
                <div className="text-center mb-8">
                    <div className="flex justify-center mb-4">
                        {branding.logoUrl ? (
                            <img src={branding.logoUrl} alt={branding.brandName} className="w-16 h-16 rounded-3xl object-cover shadow-lg" />
                        ) : (
                            <div
                                className="w-16 h-16 rounded-3xl flex items-center justify-center text-white text-xl font-bold shadow-lg"
                    ...
                    <p className="text-slate-500 dark:text-slate-400">
                        {branding.tagline || 'Entre na sua conta para continuar.'}
                    </p>
                </div>
                ...
                <p className="mt-8 text-center text-xs text-slate-400 dark:text-slate-500">
                    &copy; {new Date().getFullYear()} {branding.brandName}. Todos os direitos reservados.
                </p>
            </div>
        </div>
    )
