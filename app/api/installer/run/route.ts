...
export async function POST(req: Request) {
  ...
  const appUrl =
    ...
    normalizeAppUrl(new URL(req.url).origin) ||
  ...
  try {
    ...
    finishStep(
      'vercel_envs',
      appUrl
        ? `Environment variables configured. Public app URL set to ${appUrl}.`
    ...
    if (!schemaStatus.complete) {
      finishStepWithStatus(
        'supabase_migrations',
        'error',
        `Pending migrations: ${schemaStatus.pendingIds.join(', ')}`
      );
      return json(
        { ok: false, steps, error: `Pending migrations: ${schemaStatus.pendingIds.join(', ')}` },
        500
      );
    }
