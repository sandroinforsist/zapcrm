...
type InstallLogTone = 'info' | 'success' | 'warning' | 'error';
...
function suggestProjectName(existingNames: string[], desiredBase = 'crm-whitelabel') {
  const base = desiredBase || 'crm-whitelabel';
...
const INSTALL_STATE_STEP_ALIASES: Record<string, string> = {
  edge_secrets: 'edge_functions',
  edge_deploy: 'edge_functions',
  wait_storage: 'migrations',
  wait_vercel_deploy: 'redeploy',
};
...
export default function InstallWizardPage() {
  ...
  const lastLoggedPhaseStepRef = useRef<string | null>(null);
  ...
  const appendInstallLog = useCallback((tone: InstallLogTone, message: string) => {
  ...
  const copyInstallLogs = useCallback(async () => {
    const text = installLogs
      .map((entry) => {
        ...
        return `[${timestamp}] ${entry.tone.toUpperCase()}: ${entry.message}`;
      })
      .join('\n');
    ...
    try {
      ...
      appendInstallLog('success', 'Log da instalação copiado para a área de transferência.');
    } catch {
      appendInstallLog('warning', 'Não foi possível copiar o log automaticamente neste navegador.');
    }
  }, [appendInstallLog, installLogs]);
  ...
  // Admin e branding já foram coletados no /install/start.
  ...
  useEffect(() => {
    ...
    if (!savedToken || !savedProject || !savedUserName || !savedUserEmail || !savedBrandName) {
    ...
    try {
      ...
      setBrandSlug(savedBrandSlug || savedBrandName);
      setBrandLogoUrl(savedBrandLogo || '');
  ...
  const createProjectInOrg = async (orgSlug: string, existingNames: string[]) => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 30; attempt++) {
        const projectName = suggestProjectName(Array.from(names), brandSlug || 'crm-whitelabel');
        ...
        // Nome já existe: tenta automaticamente o próximo (crm-whitelabel -> crm-whitelabelv2 -> ...)
  ...
  const runInstaller = async () => {
    ...
    setCineProgress(0);
    setInstallLogs([]);
    lastLoggedPhaseStepRef.current = null;
    appendInstallLog('info', 'Instalação iniciada. Preparando validações iniciais.');
    ...
    commitInstallState(updateStepStatus(newInstallState, 'health_check', 'running'));
    ...
    try {
      ...
      try {
        ...
        if (healthRes.ok) {
          ...
          if (healthData.ok) {
            ...
            appendInstallLog(
              'success',
              `Health check concluído. ${healthData.pendingMigrations?.length || 0} migration(s) pendente(s).`
            );
            if (Array.isArray(healthData.pendingMigrations) && healthData.pendingMigrations.length > 0) {
              appendInstallLog(
                'info',
                `Pendentes: ${healthData.pendingMigrations.join(', ')}`
              );
            } else {
              appendInstallLog('info', 'Schema já estava em dia ou parcialmente reaproveitável.');
            }
        ...
        appendInstallLog('warning', 'Health check não respondeu. Vamos seguir com a instalação completa.');
      }
      const currentAfterHealth = installStateRef.current;
      if (currentAfterHealth) {
        commitInstallState(updateStepStatus(currentAfterHealth, 'health_check', 'completed'));
      }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          const installStateStepId = explicitStepId
            ? INSTALL_STATE_STEP_ALIASES[explicitStepId] || explicitStepId
            : null;
          
          if (event.type === 'phase') {
            const current = installStateRef.current;
            if (installStateStepId && current && current.steps.some((step) => step.id === installStateStepId)) {
              const updated = updateStepStatus(current, installStateStepId, 'running');
              commitInstallState(updated);
            }
            if (explicitStepId) {
              setCineStepLabel(STEP_LABELS[explicitStepId] || explicitStepId);
              if (lastLoggedPhaseStepRef.current !== explicitStepId) {
                appendInstallLog('info', STEP_LABELS[explicitStepId] || explicitStepId);
                lastLoggedPhaseStepRef.current = explicitStepId;
              }
            } else {
              setCineStepLabel('');
            }
            ...
            if (id) {
              ...
              appendInstallLog('info', `Deployment da Vercel iniciado: ${id}`);
            }
          } else if (event.type === 'step_complete') {
            const current = installStateRef.current;
            if (installStateStepId && current && current.steps.some((step) => step.id === installStateStepId)) {
              const updated = updateStepStatus(current, installStateStepId, 'completed');
              commitInstallState(updated);
            }
            if (event.message) {
              appendInstallLog('success', String(event.message));
            }
          } else if (event.type === 'log') {
            const tone = (event.level as InstallLogTone | undefined) || 'info';
            if (event.message) appendInstallLog(tone, String(event.message));
          } else if (event.type === 'retry') {
            const retryMsg = `Tentativa ${event.retryCount}/${event.maxRetries}...`;
            setCineSubtitle(retryMsg);
            appendInstallLog(
              'warning',
              `${STEP_LABELS[explicitStepId || ''] || explicitStepId || 'Etapa atual'}: ${retryMsg}`
            );
            console.log(`[wizard] Retry: ${event.stepId} - ${retryMsg}`);
          } else if (event.type === 'skip') {
            const current = installStateRef.current;
            if (current && Array.isArray(event.skipped)) {
              let next = current;
              for (const skipped of event.skipped) {
                const mapped = INSTALL_STATE_STEP_ALIASES[skipped] || skipped;
                if (!next.steps.some((step) => step.id === mapped)) continue;
                next = updateStepStatus(next, mapped, 'skipped');
              }
              commitInstallState(next);
              appendInstallLog('info', `Etapas reaproveitadas: ${event.skipped.join(', ')}`);
            }
            ...
            appendInstallLog('success', 'Instalação concluída com sucesso.');
            await new Promise((r) => setTimeout(r, 800));
            ...
            appendInstallLog('error', String(event.error || 'Erro durante a instalação'));
  ...
  const clearInstallerLocalData = () => {
    ...
    localStorage.removeItem(STORAGE_BRAND_NAME);
    localStorage.removeItem(STORAGE_BRAND_SLUG);
    localStorage.removeItem(STORAGE_BRAND_LOGO);
        ...
        {showInstallOverlay && (
          <motion.div key="install-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950">
            ...
            <div className="relative text-center px-4 max-w-md">
              ...
              {(installLogs.length > 0 || cinePhase === 'running' || cinePhase === 'error' || cinePhase === 'success') && (
                <div className="w-full max-w-2xl mx-auto mb-8 rounded-2xl bg-white/5 border border-white/10 overflow-hidden text-left">
                  <div className="flex items-center justify-between gap-4 px-4 py-3 border-b border-white/10">
                    <div>
                      <div className="text-white font-semibold text-sm">Log da instalação</div>
                      <div className="text-slate-500 text-xs">Use este histórico para acompanhar ou enviar ao suporte.</div>
                    </div>
                    <button
                      type="button"
                      onClick={() => void copyInstallLogs()}
                      className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-medium transition-all"
                    >
                      Copiar log
                    </button>
                  </div>
                  <div className="max-h-56 overflow-y-auto px-4 py-3 space-y-2">
                    {installLogs.length === 0 ? (
                      <div className="text-sm text-slate-500">Aguardando os primeiros eventos da instalação…</div>
                    ) : (
                      installLogs.map((entry) => {
                        ...
                        return (
                          <div key={entry.id} className="flex items-start gap-3 text-sm">
                            ...
                            <span className={toneClass}>{entry.message}</span>
                          </div>
                        );
                      })
                    )}
              ...
              {cinePhase === 'success' && (
                ...
                >
                  ...
                  <div className="rounded-2xl bg-white/5 border border-white/10 p-4 text-left space-y-2">
                    <div className="text-white font-semibold">Checklist pós-instalação</div>
                    <div className="text-sm text-slate-300">Faça o primeiro login com o admin criado no início do setup.</div>
                    <div className="text-sm text-slate-300">Confira nome, logo, pipeline e permissões do seu white-label em `Settings`.</div>
                    <div className="text-sm text-slate-300">`NEXT_PUBLIC_APP_URL` foi configurada com este domínio atual. Se você trocar de domínio depois, atualize a variável na Vercel e faça redeploy.</div>
                    <div className="text-sm text-slate-300">`CRON_SECRET` foi gerada automaticamente para proteger follow-ups e rotas agendadas.</div>
                    <div className="text-sm text-slate-300">Se for usar WhatsApp, conecte a primeira instância e teste o webhook logo após o login.</div>
                  </div>
                  <button 
                    onClick={() => {
