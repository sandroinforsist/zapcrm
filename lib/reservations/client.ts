 ...
 *
 * Connects to the reservation app tables linked to a CRM tenant.
...
export class ReservationClient {
  ...
  constructor(supabase: SupabaseClient, organizationId: string, reservationUrl = '') {
  ...
  async buildAvailabilitySummary(unitId?: string): Promise<string> {
    ...
    if (this.reservationUrl) {
      lines.push('');
      lines.push(`Link para reserva online: ${this.reservationUrl}`);
    }
...
export async function createReservationClient(
  ...
  const { data: settings } = await crmSupabase
    ...
    .select(
      'reservation_integration_enabled, reservation_supabase_url, reservation_supabase_key, reservation_url',
    )
    .eq('organization_id', organizationId)
    .single();

  if (!settings?.reservation_integration_enabled) {
    console.warn('[reservation-client] Reservation integration is disabled for this tenant');
    return null;
  }
