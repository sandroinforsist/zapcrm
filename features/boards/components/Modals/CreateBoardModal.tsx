...
type GoalType = NonNullable<Board['goal']>['type'];
...
export const CreateBoardModal: React.FC<CreateBoardModalProps> = ({
  ...
  const [goal, setGoal] = useState<Board['goal']>(undefined);
  const [agentPersona, setAgentPersona] = useState<Board['agentPersona']>(undefined);
  ...
  React.useEffect(() => {
    if (isOpen) {
      if (editingBoard) {
        ...
        setGoal(editingBoard.goal);
        setAgentPersona(editingBoard.agentPersona);
        setEntryTrigger(editingBoard.entryTrigger || '');
        ...
        try {
          ...
          if (raw) {
            ...
            if (draft && typeof draft === 'object') {
              ...
              setGoal(draft.goal ?? undefined);
              setAgentPersona(draft.agentPersona ?? undefined);
              setEntryTrigger(String(draft.entryTrigger ?? ''));
  ...
  const handleTemplateSelect = (template: BoardTemplateType | '') => {
    ...
    if (template && BOARD_TEMPLATES[template]) {
      ...
      setGoal(templateData.goal ? { ...templateData.goal } : undefined);
      setAgentPersona(templateData.agentPersona ? { ...templateData.agentPersona } : undefined);
      setEntryTrigger(templateData.entryTrigger || '');
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      goal: goal?.description || goal?.kpi || goal?.targetValue ? {
      ...
      agentPersona:
        agentPersona?.name || agentPersona?.role || agentPersona?.behavior
          ? {
              name: agentPersona?.name || '',
              role: agentPersona?.role || '',
              behavior: agentPersona?.behavior || '',
            }
          : undefined,
        ...
        <div className="flex flex-col">
          ...
          */}
          <div className="overflow-y-auto p-4 sm:p-6 space-y-6 max-h-[calc(100dvh-14rem)] sm:max-h-[calc(100dvh-18rem)]">
              ...
              {/* Goal / Objective */}
              <div className="rounded-2xl border border-slate-200 dark:border-white/10 p-4 bg-slate-50/60 dark:bg-black/20">
                <div className="mb-4">
                  <h4 className="text-sm font-semibold text-slate-900 dark:text-white">
                    Objetivo do pipeline
                  </h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Essa meta orienta o tenant e também alimenta a estratégia da I.A.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setGoal((current) => ({
                          ...(current || { description: '', kpi: '', targetValue: '' }),
                          kpi: e.target.value,
                        }))
                      }
                      placeholder="Ex: Receita nova, número de reservas, taxa de conversão"
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setGoal((current) => ({
                          ...(current || { description: '', kpi: '', targetValue: '' }),
                          type: (e.target.value || undefined) as GoalType,
                        }))
                      }
                    ...
                    >
                      ...
                      <option value="currency">Receita</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ex: Converter leads em reuniões qualificadas"
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 dark:border-white/10 p-4 bg-slate-50/60 dark:bg-black/20">
                <div className="mb-4">
                  <h4 className="text-sm font-semibold text-slate-900 dark:text-white">
                    Agente e regra de entrada
                  </h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Defina como a I.A. deve atuar e quais leads entram nesse pipeline.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setAgentPersona((current) => ({
                          ...(current || { name: '', role: '', behavior: '' }),
                          name: e.target.value,
                        }))
                      }
                      placeholder="Ex: SDR Bot"
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Ex: Closer, pré-vendas, concierge"
                      className="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    />
                  </div>
                </div>
                ...
                <div className="mt-3">
                  <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-2">
                    Comportamento da I.A.
                  </label>
                  <textarea
                    value={agentPersona?.behavior || ''}
                    onChange={(e) =>
                      setAgentPersona((current) => ({
                        ...(current || { name: '', role: '', behavior: '' }),
                    ...
                    placeholder="Ex: Seja consultivo, rápido, pergunte orçamento e urgência antes de avançar."
                    className="w-full h-24 px-4 py-3 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none"
                  />
                </div>
                ...
                <div className="mt-3">
                  ...
                  <textarea
                    ...
                    placeholder="Ex: Entram aqui leads vindos do WhatsApp com orçamento validado."
                    className="w-full h-20 px-4 py-3 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none"
                  />
                </div>
              </div>
